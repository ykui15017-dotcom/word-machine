...
export function validateScenePlan(plan){
  ...
  if(['concept','visual','scale'].includes(plan?.subject?.category))errors.push(`${plan.subject.category}_as_subject`);
  const used=new Set(plan?.usedWordIds||[]),raw=plan?.ingredients||[];
  if(raw.some(word=>word.category==='observation'&&!used.has(word.id)))errors.push('unused_observation');
