...
let state=getMachineState(),mixMode='short';
...
if(incoming){const base=newDrop(4,[{...incoming,locked:true}]);state={...base,recipeId:base.recipe.id}}
if(!state?.words?.length){const base=newDrop(4);state={...base,recipeId:base.recipe.id}}
function persist(){setMachineState({recipe:state.recipe,recipeId:state.recipe.id,words:state.words})}
...
function drop(freshRecipe=true,count=3+Math.floor(Math.random()*3)){
  ...
  state=newDrop(count,locked,freshRecipe?null:state.recipe);
...
document.querySelector('.actions').addEventListener('click',event=>{
  ...
  if(action==='again'){state=replaceUnlocked(state.words,state.recipe);recipeBox.hidden=true;render(true);say('same recipe, new words.')}
  if(action==='more'){state=addWord(state.words,state.recipe);render(true)}
  if(action==='less'&&state.words.length>3){const index=[...state.words].map((word,i)=>({word,i})).reverse().find(item=>!item.word.locked)?.i;if(index!==undefined)state.words.splice(index,1);render()}
